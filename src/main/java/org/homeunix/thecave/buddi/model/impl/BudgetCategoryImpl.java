...
public class BudgetCategoryImpl extends SourceImpl implements BudgetCategory {
    ...
    public Map<String, Long> getAmounts() {
        if (amounts == null) amounts = new HashMap<String, Long>();
        return amounts;
    }
    ...
    public void setDeleted(boolean deleted) throws InvalidValueException {
        if (getDocument() != null) getDocument().startBatchChange();
        ...
        if (getDocument() != null) {
            if (deleted) {
                ...
                if (getParent() != null) getParent().setDeleted(deleted);
            }
        }
        ...
        if (getDocument() != null) getDocument().finishBatchChange();
    }
    ...
    public long getAmount(Date startDate, Date endDate) {
        ...
        BudgetPeriod budgetPeriod = firstBudgetPeriod;
        while (budgetPeriod.before(lastBudgetPeriod)) {
            totalInMiddle += getBudgetInPeriod(new Period(startDate, endDate), budgetPeriod);
    ...
    public void setAmount(Date periodDate, long amount) {
        if (getAmount(periodDate) != amount) setChanged();
...
//		}
    ...
    private String getPeriodKey(Date periodDate) {
        Date d = BudgetPeriod.createBudgetPeriod(periodDate, this.getBudgetPeriodType()).getStartDate();
    ...
    private Date getPeriodDate(String periodKey) {
        ...
        if (splitKey.length == 4) {
            ...
            return BudgetPeriod.createBudgetPeriod(DateUtil.getDate(year, month, day), this.getBudgetPeriodType()).getStartDate();
        }
    ...
    public BudgetCategoryType getBudgetPeriodType() {
        if (getPeriodType() == null) setPeriodType(new BudgetCategoryTypeMonthly());
    ...
    public List<Date> getBudgetedDates() {
        ...
        for (String key : amounts.keySet()) {
            if (amounts.get(key) != null && amounts.get(key) != 0) budgetedDates.add(getPeriodDate(key));
        }
    ...
    BudgetCategory clone(Map<ModelObject, ModelObject> originalToCloneMap) throws CloneNotSupportedException {
        ...
        if (originalToCloneMap.get(this) != null) return (BudgetCategory) originalToCloneMap.get(this);
        ...
        b.name = name;
        b.notes = notes;
        if (parent != null) b.parent = (BudgetCategory) ((BudgetCategoryImpl) parent).clone(originalToCloneMap);
