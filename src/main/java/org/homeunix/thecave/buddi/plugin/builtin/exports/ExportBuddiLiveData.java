...
public class ExportBuddiLiveData extends BuddiExportPlugin {
	...
	public void exportData(ImmutableDocument model, MossDocumentFrame callingFrame, File file) throws PluginException, PluginMessage {
		try {
			...
			for (ImmutableAccount account : model.getImmutableAccounts()) {
				...
				a.put("uuid", account.getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", "").replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
			...
			for (ImmutableBudgetCategory category : model.getImmutableBudgetCategories()) {
				for (Date d : category.getBudgetedDates()) {
					...
					e.put("category", category.getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
			...
			for (ScheduledTransaction scheduledTransaction : ((Document) model.getRaw().getDocument()).getScheduledTransactions()){
				...
				st.put("uuid", scheduledTransaction.getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
				...
				if (scheduledTransaction.getFromSplits().size() == 0 && scheduledTransaction.getToSplits().size() == 0){
					...
					s.put("from", scheduledTransaction.getFrom().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
					s.put("to", scheduledTransaction.getTo().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
				...
				// one, with the other side being the single source
					...
					for (TransactionSplit split : fromSplits ? scheduledTransaction.getFromSplits() : scheduledTransaction.getToSplits()) {
						...
						s.put("from", fromSplits ? split.getSource().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", "") : scheduledTransaction.getFrom().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
						s.put("to", !fromSplits ? split.getSource().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", "") : scheduledTransaction.getTo().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
						s.put("memo", scheduledTransaction.getMemo());
						st.append("splits", s);
					}
				}
			...
			for (ImmutableTransaction transaction : model.getImmutableTransactions()) {
				...
				t.put("uuid", transaction.getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
				//Here we have to do some work, since the data models differ a bit.
				// If there are no splits, then our life is easy: just add a single
				// split with the from / to sources as indicated.
				if (transaction.getImmutableFromSplits().size() == 0 && transaction.getImmutableToSplits().size() == 0){
					...
					s.put("from", transaction.getFrom().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
					s.put("to", transaction.getTo().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
				...
				// one, with the other side being the single source
					...
					for (ImmutableTransactionSplit split : fromSplits ? transaction.getImmutableFromSplits() : transaction.getImmutableToSplits()) {
						final JSONObject s = new JSONObject();
						s.put("amount", formatCurrency(new BigDecimal(split.getAmount()).divide(new BigDecimal(100))));
						s.put("from", fromSplits ? split.getSource().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", "") : transaction.getFrom().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
						s.put("to", !fromSplits ? split.getSource().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", "") : transaction.getTo().getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
						s.put("memo", transaction.getMemo());
						t.append("splits", s);
					}
				}
	...
	private JSONArray getCategories(List<ImmutableBudgetCategory> categories) throws JSONException {
		...
		for (ImmutableBudgetCategory category : categories) {
			...
			c.put("uuid", category.getUid().replaceAll("org.homeunix.thecave.org.homeunix.thecave.buddi.model.impl.*Impl-", ""));
